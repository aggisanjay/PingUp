              ...
              <div className='flex flex-wrap max-w-6xl'>
                {
                  posts.filter((post)=>post.image_urls.length>0).map((post)=>(
                    ...
                    {post.image_urls.map((image,index)=>(
                      ...
                      <p className='absolute bottom-0 right-0 text-xs p-1 px-3 backdrop-blur-xl tex-white opacity-0 group-hover:opacity-100 transition duration-300'>Posted {moment(post.createAt).fromNow()}</p>
                      </Link>
                    ))}
                    </>
                  ))
